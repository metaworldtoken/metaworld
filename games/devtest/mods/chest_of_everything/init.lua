...
local inv_everything = metaworld.create_detached_inventory("everything", {
...
local inv_trash = metaworld.create_detached_inventory("trash", {
	allow_take = function(inv, listname, index, stack, player)
		return 0
	end,
	allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
...
metaworld.register_node("chest_of_everything:chest", {
...
metaworld.register_on_mods_loaded(function()
	local items = {}
	for itemstring,_ in pairs(metaworld.registered_items) do
	...
	* Dummy items ]]
	local function compare(item1, item2)
		local def1 = metaworld.registered_items[item1]
		local def2 = metaworld.registered_items[item2]
		local tool1 = def1.type == "tool"
		local tool2 = def2.type == "tool"
		local testtool1 = metaworld.get_item_group(item1, "testtool") == 1
		local testtool2 = metaworld.get_item_group(item2, "testtool") == 1
		local dummy1 = metaworld.get_item_group(item1, "dummy") == 1
		local dummy2 = metaworld.get_item_group(item2, "dummy") == 1
