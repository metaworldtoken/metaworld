...
local F = metaworld.formspec_escape

dofile(metaworld.get_modpath("testtools") .. "/light.lua")

metaworld.register_tool("testtools:param2tool", {
	...
	on_use = function(itemstack, user, pointed_thing)
		local pos = metaworld.get_pointed_thing_position(pointed_thing)
...
metaworld.register_tool("testtools:node_setter", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.type == "nothing" then
			...
			if user and user:is_player() then
				metaworld.chat_send_player(user:get_player_name(), S("Now placing: @1 (param2=@2)", "air", 0))
			end
			return itemstack
		...
		if user and user:is_player() then
			metaworld.chat_send_player(user:get_player_name(), S("Now placing: @1 (param2=@2)", node.name, node.param2))
		end
		return itemstack
	end,
	on_secondary_use = function(itemstack, user, pointed_thing)
		...
		metaworld.show_formspec(user:get_player_name(), "testtools:node_setter",
			"size[4,4]"..
	...
	on_place = function(itemstack, user, pointed_thing)
		...
		if nodename == "" and user and user:is_player() then
			metaworld.chat_send_player(user:get_player_name(), S("Punch a node first!"))
			return
		end
		...
		if not metaworld.registered_nodes[nodename] then
			metaworld.chat_send_player(user:get_player_name(), S("Cannot set unknown node: @1", nodename))
			return
		end
		metaworld.set_node(pos, node)
	end,
})

metaworld.register_tool("testtools:remover", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.type == "node" and pos ~= nil then
			metaworld.remove_node(pos)
		elseif pointed_thing.type == "object" then
			local obj = pointed_thing.ref
			if not obj:is_player() then
				obj:remove()
			else
				metaworld.chat_send_player(user:get_player_name(), S("Can't remove players!"))
			end
		end
	end,
})

metaworld.register_tool("testtools:falling_node_tool", {
	...
	on_place = function(itemstack, user, pointed_thing)
		...
		for i=1,2 do
			...
			local n2 = metaworld.get_node(above)
			local def2 = metaworld.registered_nodes[n2.name]
		...
		if highest then
			local node = metaworld.get_node(pos)
			local metatable = metaworld.get_meta(pos):to_table()
			metaworld.remove_node(pos)
			metaworld.set_node(highest, node)
			local meta_highest = metaworld.get_meta(highest)
			meta_highest:from_table(metatable)
			ok = metaworld.spawn_falling_node(highest)
		else
			ok = metaworld.spawn_falling_node(pos)
		end
		if not ok and user and user:is_player() then
			metaworld.chat_send_player(user:get_player_name(), S("Falling node could not be spawned!"))
		end
	end,
	on_use = function(itemstack, user, pointed_thing)
		local pos = metaworld.get_pointed_thing_position(pointed_thing)
		...
		local ok = metaworld.spawn_falling_node(pos)
		if not ok and user and user:is_player() then
			metaworld.chat_send_player(user:get_player_name(), S("Falling node could not be spawned!"))
		end
	end,
...
metaworld.register_tool("testtools:rotator", {
...
local mover_config = function(itemstack, user, pointed_thing)
	...
	metaworld.chat_send_player(user:get_player_name(), S("distance=@1/10", dist*2))
	return itemstack
end

metaworld.register_tool("testtools:object_mover", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local dir = metaworld.yaw_to_dir(yaw)
...
metaworld.register_tool("testtools:entity_scaler", {
...
local function get_entity_list()
	...
	local ents = metaworld.registered_entities
	local list = {}
...
metaworld.register_tool("testtools:entity_spawner", {
	...
	on_place = function(itemstack, user, pointed_thing)
		...
		if pointed_thing.type == "node" then
			if selections[name] then
				...
				metaworld.add_entity(pos, get_entity_list()[selections[name]])
			else
				metaworld.chat_send_player(name, S("Select an entity first (with punch key)!"))
			end
		end
	end,
	on_use = function(itemstack, user, pointed_thing)
		...
		if user and user:is_player() then
			...
			metaworld.show_formspec(name, "testtools:entity_list",
				"size[9,9]"..
...
local editor_formspec = function(playername, obj, value, sel)
	...
	metaworld.show_formspec(playername, "testtools:object_editor",
		"size[9,9]"..
		"label[0,0;"..F(title).."]"..
...
metaworld.register_tool("testtools:object_editor", {
...
local attacher_config = function(itemstack, user, pointed_thing)
	...
	if ctrl.aux1 then
		...
		metaworld.chat_send_player(name, S("rotation=@1", metaworld.pos_to_string({x=rot_x,y=0,z=0})))
		...
		metaworld.chat_send_player(name, S("position=@1", metaworld.pos_to_string({x=0,y=pos_y,z=0})))
		meta:set_int("pos_y", pos_y)
	end
	return itemstack
end

metaworld.register_tool("testtools:object_attacher", {
		...
		if user and user:is_player() then
			...
			if ctrl.sneak then
				if selected_object:get_attach() then
					...
					metaworld.chat_send_player(name, S("Object detached!"))
				else
					metaworld.chat_send_player(name, S("Object is not attached!"))
				end
				return
			end
			...
			if selected_object == parent then
				metaworld.chat_send_player(name, S("Parent object selected: @1", ename))
			elseif selected_object == child then
				metaworld.chat_send_player(name, S("Child object selected: @1", ename))
			end
			if parent and child then
				if parent == child then
					metaworld.chat_send_player(name, S("Can't attach an object to itself!"))
					ent_parent[name] = nil
					ent_child[name] = nil
					return
				end
				...
				if check_parent then
					metaworld.chat_send_player(name, S("Object attached! position=@1, rotation=@2",
						metaworld.pos_to_string(offset), metaworld.pos_to_string(angle)))
				else
					metaworld.chat_send_player(name, S("Attachment failed!"))
				end
				ent_parent[name] = nil
				ent_child[name] = nil
			end
...
metaworld.register_tool("testtools:children_getter", {
	...
	on_use = function(itemstack, user, pointed_thing)
		if user and user:is_player() then
			...
			metaworld.chat_send_player(user:get_player_name(), ret)
		end
	end,
})
...
local function use_loadstring(param, player)
	...
	local privs = metaworld.get_player_privs(player:get_player_name())
...
local function show_node_meta_formspec(user, pos, key, value, keylist)
	...
	metaworld.show_formspec(user:get_player_name(),
		...
		"label[0,7.2;"..F(S("pos = @1", metaworld.pos_to_string(pos))).."]")
end
...
metaworld.register_tool("testtools:node_meta_editor", {
...
metaworld.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "testtools:entity_list" then
		...
		if fields.entity_list then
			local expl = metaworld.explode_textlist_event(fields.entity_list)
			if expl.type == "DCL" then
				local pos = vector.add(player:get_pos(), {x=0,y=1,z=0})
				selections[name] = expl.index
				metaworld.add_entity(pos, get_entity_list()[expl.index])
				return
	...
	elseif formname == "testtools:object_editor" then
		...
		if fields.object_props then
			local expl = metaworld.explode_textlist_event(fields.object_props)
		...
		if fields.key_enter_field == "value" or fields.submit then
			...
			if success then
				...
				metaworld.chat_send_player(name, str)
				return
			end
	...
	elseif formname == "testtools:node_meta_editor" then
		...
		if fields.keylist then
			local evnt = metaworld.explode_textlist_event(fields.keylist)
...
metaworld.register_on_leaveplayer(function(player)
	local name = player:get_player_name()
