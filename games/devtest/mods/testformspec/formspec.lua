local color = metaworld.colorize
...
local hypertext_fs = "hypertext[0,0;11,9;hypertext;"..metaworld.formspec_escape(hypertext_basic).."]"..
	"hypertext[0,9.5;11,2.5;hypertext;"..metaworld.formspec_escape(hypertext_global).."]"
...
local function show_test_formspec(pname, page_id)
	...
	metaworld.show_formspec(pname, "testformspec:formspec", fs)
end
...
metaworld.register_on_player_receive_fields(function(player, formname, fields)
	...
	if fields.ani_img_1 and fields.ani_btn_1 then
		metaworld.chat_send_player(player:get_player_name(), "ani_img_1 = " .. tostring(fields.ani_img_1))
		return true
	elseif fields.ani_img_2 and fields.ani_btn_2 then
		metaworld.chat_send_player(player:get_player_name(), "ani_img_2 = " .. tostring(fields.ani_img_2))
		return true
	end

	if fields.hypertext then
		metaworld.chat_send_player(player:get_player_name(), "Hypertext action received: " .. tostring(fields.hypertext))
		return true
	end
end)

metaworld.register_chatcommand("test_formspec", {
	params = "",
	description = "Open the test formspec",
	func = function(name)
		if not metaworld.get_player_by_name(name) then
