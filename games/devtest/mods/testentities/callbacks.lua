...
local spos = function(self)
	return metaworld.pos_to_string(vector.round(self.object:get_pos()))
end

-- Callback test entity (all callbacks except on_step)
metaworld.register_entity("testentities:callback", {
	initial_properties = {
...
metaworld.register_entity("testentities:callback_step", {
	visual = "upright_sprite",
