...
metaworld.register_globalstep(function(dtime)
	etime = etime + dtime
	if etime < 1 then
		return
	end
	etime = 0
	for _, player in ipairs(metaworld.get_connected_players()) do
...
metaworld.register_chatcommand("hudfonts", {
	params = "",
	...
	func = function(name, param)
		...
		if player_huds[name] == nil then
			...
			metaworld.chat_send_player(name, ("%d HUD element(s) added."):format(param))
		else
			...
			if huds then
				...
				metaworld.chat_send_player(name, "All HUD elements removed.")
			end
