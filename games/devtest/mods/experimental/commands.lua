metaworld.register_chatcommand("test_inv", {
	params = "",
...
metaworld.register_chatcommand("test_bulk_set_node", {
	params = "",
	...
	func = function(name, param)
		...
		metaworld.bulk_set_node(pos_list, {name = "mapgen_stone"})
		return true, "Done."
	end,
})

metaworld.register_chatcommand("bench_bulk_set_node", {
	params = "",
	...
	func = function(name, param)
		...
		metaworld.chat_send_player(name, "Benchmarking metaworld.bulk_set_node. Warming up ...");

		-- warm up with stone to prevent having different callbacks
		-- due to different node topology
		metaworld.bulk_set_node(pos_list, {name = "mapgen_stone"})

		metaworld.chat_send_player(name, "Warming up finished, now benchmarking ...");

		local start_time = metaworld.get_us_time()
		for i=1,#pos_list do
			metaworld.set_node(pos_list[i], {name = "mapgen_stone"})
		end
		local middle_time = metaworld.get_us_time()
		metaworld.bulk_set_node(pos_list, {name = "mapgen_stone"})
		local end_time = metaworld.get_us_time()
		local msg = string.format("Benchmark results: metaworld.set_node loop: %.2f ms; metaworld.bulk_set_node: %.2f ms",
...
local function place_nodes(param)
	...
	table.sort(nodes)
	metaworld.chat_send_player(name, "Placing nodes …")
	local nodes_placed = 0
	local aborted = false
	for n=1, #nodes do
		local itemstring = nodes[n]
		local def = metaworld.registered_nodes[itemstring]
		local p2_max = 0
		...
		for p2 = 0, p2_max do
			...
			if not ((def.paramtype2 == "meshoptions" and p2 % 8 > 4) or
				...
				metaworld.set_node(pos, { name = itemstring, param2 = p2 })
	...
	if aborted then
		metaworld.chat_send_player(name, "Not all nodes could be placed, please move further away from the world boundary. Nodes placed: "..nodes_placed)
	end
	metaworld.chat_send_player(name, "Nodes placed: "..nodes_placed..".")
end
...
metaworld.register_chatcommand("test_place_nodes", {
	params = "[ no_param2 ]",
	...
	func = function(name, param)
		...
		for itemstring, def in pairs(metaworld.registered_nodes) do
		...
		metaworld.emerge_area(start_pos,
...
core.register_on_chatcommand(function(name, command, params)
	metaworld.log("action", "caught command '"..command.."', issued by '"..name.."'. Parameters: '"..params.."'")
end)
