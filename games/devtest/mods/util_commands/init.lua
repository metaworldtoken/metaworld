metaworld.register_chatcommand("hotbar", {
	params = "<size>",
...
metaworld.register_chatcommand("hp", {
	params = "<hp>",
...
metaworld.register_chatcommand("zoomfov", {
	params = "[<FOV>]",
...
local s_infplace = metaworld.settings:get("devtest_infplace")
if s_infplace == "true" then
	infplace = true
elseif s_infplace == "false" then
	infplace = false
else
	infplace = metaworld.is_creative_enabled("")
end

metaworld.register_chatcommand("infplace", {
	params = "",
	description = "Toggle infinite node placement",
	func = function(name, param)
		infplace = not infplace
		if infplace then
			metaworld.chat_send_all("Infinite node placement enabled!")
			metaworld.log("action", "Infinite node placement enabled")
		else
			metaworld.chat_send_all("Infinite node placement disabled!")
			metaworld.log("action", "Infinite node placement disabled")
		end
		return true
	end,
})

metaworld.register_chatcommand("detach", {
	params = "[<radius>]",
	...
	func = function(name, param)
		...
		local objs = metaworld.get_objects_inside_radius(player:get_pos(), radius)
		local num = 0
		for o=1, #objs do
...
metaworld.register_chatcommand("use_tool", {
	...
	func = function(name, param)
		...
		for u=1, uses do
			...
			local dp
			if mode == "dig" then
				dp = metaworld.get_dig_params({[group]=3, level=level}, caps, wear)
			else
				dp = metaworld.get_hit_params({[group]=100}, caps, level, wear)
			end
...
metaworld.register_chatcommand("test_waypoints", {
	...
	func = function(name, params)
		...
		metaworld.chat_send_player(name, "Waypoint IDs: regular: " .. regular .. ", reduced precision: " .. reduced_precision ..
			", hidden distance: " .. hidden_distance .. ", image waypoint: " .. image_waypoint)
	end
})

-- Unlimited node placement
metaworld.register_on_placenode(function(pos, newnode, placer, oldnode, itemstack)
...
local old_handle_node_drops = metaworld.handle_node_drops
function metaworld.handle_node_drops(pos, drops, digger)
