...
[ -e $metaworld ] || { echo "executable $metaworld missing"; exit 1; }
...
gdbrun $metaworld --server --config $conf_server --world $worldpath --gameid $gameid 2>&1 | sed -u 's/^/(server) /' &
waitfor $worldpath/startup

echo "Starting client"
gdbrun $metaworld --config $conf_client1 --go --address 127.0.0.1 2>&1 | sed -u 's/^/(client) /' &
