-- metaworld.lua
-- Packet dissector for the UDP-based metaworld protocol
...
function metaworld_field_helper(lentype, name, abbr)
...
metaworld_client_commands = {}
metaworld_client_obsolete = {}

-- TOSERVER_INIT

do
	local abbr = "metaworld.client.init_"
	...
	local f_player_namelen, f_player_name =
		metaworld_field_helper("uint16", abbr.."player_name", "Player Name")

	metaworld_client_commands[0x02] = {
		"INIT",                            -- Command name
		...
		function(buffer, pinfo, tree, t)   -- Dissector function [optional]
			...
			metaworld_decode_helper_ascii(buffer, t, "uint16", 9, f_player_namelen, f_player_name)
		end
	}
end
...
metaworld_client_commands[0x10] = { "INIT_LEGACY", 53 }
metaworld_client_obsolete[0x10] = true

-- TOSERVER_INIT2

do
	local f_langlen, f_lang =
		metaworld_field_helper("uint16", "metaworld.client.init2_language", "Language Code")

	metaworld_client_commands[0x11] = {
		"INIT2",
		2,
		{ f_langlen,
		  f_lang },
		function(buffer, pinfo, tree, t)
			metaworld_decode_helper_ascii(buffer, t, "uint16", 2, f_langlen, f_lang)
		end
	}
end
...
metaworld_client_commands[0x17] = { "MODCHANNEL_JOIN", 2 }

-- TOSERVER_MODCHANNEL_LEAVE

metaworld_client_commands[0x18] = { "MODCHANNEL_LEAVE", 2 }

-- TOSERVER_MODCHANNEL_MSG

metaworld_client_commands[0x19] = { "MODCHANNEL_MSG", 2 }

-- TOSERVER_GETBLOCK (obsolete)

metaworld_client_commands[0x20] = { "GETBLOCK", 2 }
metaworld_client_obsolete[0x20] = true

-- TOSERVER_ADDNODE (obsolete)

metaworld_client_commands[0x21] = { "ADDNODE", 2 }
metaworld_client_obsolete[0x21] = true

-- TOSERVER_REMOVENODE (obsolete)

metaworld_client_commands[0x22] = { "REMOVENODE", 2 }
metaworld_client_obsolete[0x22] = true

-- TOSERVER_PLAYERPOS

do
	local abbr = "metaworld.client.playerpos_"
	...
	metaworld_client_commands[0x23] = {
		"PLAYERPOS", 34,
...
do
	local f_count = ProtoField.uint8("metaworld.client.gotblocks_count", "Count", base.DEC)
	local f_block = ProtoField.bytes("metaworld.client.gotblocks_block", "Block", base.NONE)
	local f_x = ProtoField.int16("metaworld.client.gotblocks_x", "Block position X", base.DEC)
	local f_y = ProtoField.int16("metaworld.client.gotblocks_y", "Block position Y", base.DEC)
	local f_z = ProtoField.int16("metaworld.client.gotblocks_z", "Block position Z", base.DEC)

	metaworld_client_commands[0x24] = {
		"GOTBLOCKS", 3,
		{ f_count, f_block, f_x, f_y, f_z },
		function(buffer, pinfo, tree, t)
			t:add(f_count, buffer(2,1))
			local count = buffer(2,1):uint()
			if metaworld_check_length(buffer, 3 + 6*count, t) then
...
do
	local f_count = ProtoField.uint8("metaworld.client.deletedblocks_count", "Count", base.DEC)
	local f_block = ProtoField.bytes("metaworld.client.deletedblocks_block", "Block", base.NONE)
	local f_x = ProtoField.int16("metaworld.client.deletedblocks_x", "Block position X", base.DEC)
	local f_y = ProtoField.int16("metaworld.client.deletedblocks_y", "Block position Y", base.DEC)
	local f_z = ProtoField.int16("metaworld.client.deletedblocks_z", "Block position Z", base.DEC)

	metaworld_client_commands[0x25] = {
		"DELETEDBLOCKS", 3,
		{ f_count, f_block, f_x, f_y, f_z },
		function(buffer, pinfo, tree, t)
			t:add(f_count, buffer(2,1))
...
metaworld_client_commands[0x26] = { "ADDNODE_FROM_INVENTORY", 2 }
metaworld_client_obsolete[0x26] = true

-- TOSERVER_CLICK_OBJECT (obsolete)

metaworld_client_commands[0x27] = { "CLICK_OBJECT", 2 }
metaworld_client_obsolete[0x27] = true

-- TOSERVER_GROUND_ACTION (obsolete)

metaworld_client_commands[0x28] = { "GROUND_ACTION", 2 }
metaworld_client_obsolete[0x28] = true

-- TOSERVER_RELEASE (obsolete)

metaworld_client_commands[0x29] = { "RELEASE", 2 }
metaworld_client_obsolete[0x29] = true

-- TOSERVER_SIGNTEXT (obsolete)

metaworld_client_commands[0x30] = { "SIGNTEXT", 2 }
metaworld_client_obsolete[0x30] = true

-- TOSERVER_INVENTORY_ACTION

do
	local f_action = ProtoField.string("metaworld.client.inventory_action", "Action")

	metaworld_client_commands[0x31] = {
		"INVENTORY_ACTION", 2,
		{ f_action },
		function(buffer, pinfo, tree, t)
...
do
	local f_length = ProtoField.uint16("metaworld.client.chat_message_length", "Length", base.DEC)
	local f_message = ProtoField.string("metaworld.client.chat_message", "Message")

	metaworld_client_commands[0x32] = {
		"CHAT_MESSAGE", 4,
		{ f_length, f_message },
		function(buffer, pinfo, tree, t)
			t:add(f_length, buffer(2,2))
			local textlen = buffer(2,2):uint()
			if metaworld_check_length(buffer, 4 + textlen*2, t) then
...
metaworld_client_commands[0x33] = { "SIGNNODETEXT", 2 }
metaworld_client_obsolete[0x33] = true


-- TOSERVER_CLICK_ACTIVEOBJECT (obsolete)

metaworld_client_commands[0x34] = { "CLICK_ACTIVEOBJECT", 2 }
metaworld_client_obsolete[0x34] = true

-- TOSERVER_DAMAGE

do
	local f_amount = ProtoField.uint8("metaworld.client.damage_amount", "Amount", base.DEC)

	metaworld_client_commands[0x35] = {
		"DAMAGE", 3,
		{ f_amount },
		function(buffer, pinfo, tree, t)
...
metaworld_client_commands[0x36] = { "CLICK_ACTIVEOBJECT", 2 }
metaworld_client_obsolete[0x36] = true

-- TOSERVER_PLAYERITEM

do
	local f_item = ProtoField.uint16("metaworld.client.playeritem_item", "Wielded item")

	metaworld_client_commands[0x37] = {
		"PLAYERITEM", 4,
		{ f_item },
		function(buffer, pinfo, tree, t)
...
metaworld_client_commands[0x38] = { "RESPAWN", 2 }

-- TOSERVER_INTERACT

do
	local abbr = "metaworld.client.interact_"
	local vs_action = {
		[0] = "Start digging",
		[1] = "Stop digging",
		[2] = "Digging completed",
	...
	metaworld_client_commands[0x39] = {
		"INTERACT", 11,
		{ f_action,
		  f_item,
		  f_plen,
		...
		function(buffer, pinfo, tree, t)
			...
			if metaworld_check_length(buffer, 9 + plen, t) then
...
metaworld_client_commands[0x3a] = { "REMOVED_SOUNDS", 2 }
metaworld_client_commands[0x3b] = { "NODEMETA_FIELDS", 2 }
metaworld_client_commands[0x3c] = { "INVENTORY_FIELDS", 2 }
metaworld_client_commands[0x40] = { "REQUEST_MEDIA", 2 }
metaworld_client_commands[0x41] = { "RECEIVED_MEDIA", 2 }

-- TOSERVER_BREATH (obsolete)

metaworld_client_commands[0x42] = { "BREATH", 2 }
metaworld_client_obsolete[0x42] = true

-- TOSERVER_CLIENT_READY

do
	local abbr = "metaworld.client.client_ready_"
	...
	local f_versionlen, f_version =
		metaworld_field_helper("uint16", abbr.."version", "Full Version String")
	local f_formspec_ver = ProtoField.uint16(abbr.."formspec_version",
		"Formspec API version")

	metaworld_client_commands[0x43] = {
		"CLIENT_READY",
		8,
		...
		function(buffer, pinfo, tree, t)
			...
			local off = metaworld_decode_helper_ascii(buffer, t, "uint16", 6,
				f_versionlen, f_version)
			if off and metaworld_check_length(buffer, off + 2, t) then
...
metaworld_client_commands[0x50] = { "FIRST_SRP", 2 }
metaworld_client_commands[0x51] = { "SRP_BYTES_A", 2 }
metaworld_client_commands[0x52] = { "SRP_BYTES_M", 2 }
...
metaworld_server_commands = {}
metaworld_server_obsolete = {}

-- TOCLIENT_HELLO

do
	local abbr = "metaworld.server.hello_"
	...
	local f_legacy_namelen, f_legacy_name = metaworld_field_helper("uint16",
		abbr.."legacy_name", "Legacy player name for hashing")

	metaworld_server_commands[0x02] = {
		"HELLO",
		13,
		...
		function(buffer, pinfo, tree, t)
			...
			metaworld_decode_helper_ascii(buffer, t, "uint16", 11, f_legacy_namelen, f_legacy_name)
		end
	}
end
...
do
	local abbr = "metaworld.server.auth_accept_"
	...
	metaworld_server_commands[0x03] = {
		"AUTH_ACCEPT",
		30,
...
metaworld_server_commands[0x04] = {"ACCEPT_SUDO_MODE", 2}
metaworld_server_commands[0x05] = {"DENY_SUDO_MODE", 2}
metaworld_server_commands[0x0A] = {"ACCESS_DENIED", 2}

-- TOCLIENT_INIT (obsolete)

metaworld_server_commands[0x10] = { "INIT", 2 }
metaworld_server_obsolete[0x10] = true

-- TOCLIENT_BLOCKDATA

do
	local f_x = ProtoField.int16("metaworld.server.blockdata_x", "Block position X", base.DEC)
	local f_y = ProtoField.int16("metaworld.server.blockdata_y", "Block position Y", base.DEC)
	local f_z = ProtoField.int16("metaworld.server.blockdata_z", "Block position Z", base.DEC)
	local f_data = ProtoField.bytes("metaworld.server.blockdata_block", "Serialized MapBlock")

	metaworld_server_commands[0x20] = {
		"BLOCKDATA", 8,
		{ f_x, f_y, f_z, f_data },
		function(buffer, pinfo, tree, t)
...
do
	local f_x = ProtoField.int16("metaworld.server.addnode_x", "Position X", base.DEC)
	local f_y = ProtoField.int16("metaworld.server.addnode_y", "Position Y", base.DEC)
	local f_z = ProtoField.int16("metaworld.server.addnode_z", "Position Z", base.DEC)
	local f_data = ProtoField.bytes("metaworld.server.addnode_node", "Serialized MapNode")

	metaworld_server_commands[0x21] = {
		"ADDNODE", 8,
		{ f_x, f_y, f_z, f_data },
		function(buffer, pinfo, tree, t)
			t:add(f_x, buffer(2,2))
...
do
	local f_x = ProtoField.int16("metaworld.server.removenode_x", "Position X", base.DEC)
	local f_y = ProtoField.int16("metaworld.server.removenode_y", "Position Y", base.DEC)
	local f_z = ProtoField.int16("metaworld.server.removenode_z", "Position Z", base.DEC)

	metaworld_server_commands[0x22] = {
		"REMOVENODE", 8,
		{ f_x, f_y, f_z },
		function(buffer, pinfo, tree, t)
			t:add(f_x, buffer(2,2))
...
metaworld_server_commands[0x23] = { "PLAYERPOS", 2 }
metaworld_server_obsolete[0x23] = true

-- TOCLIENT_PLAYERINFO (obsolete)

metaworld_server_commands[0x24] = { "PLAYERINFO", 2 }
metaworld_server_obsolete[0x24] = true

-- TOCLIENT_OPT_BLOCK_NOT_FOUND (obsolete)

metaworld_server_commands[0x25] = { "OPT_BLOCK_NOT_FOUND", 2 }
metaworld_server_obsolete[0x25] = true

-- TOCLIENT_SECTORMETA (obsolete)

metaworld_server_commands[0x26] = { "SECTORMETA", 2 }
metaworld_server_obsolete[0x26] = true

-- TOCLIENT_INVENTORY

do
	local f_inventory = ProtoField.string("metaworld.server.inventory", "Inventory")

	metaworld_server_commands[0x27] = {
		"INVENTORY", 2,
		{ f_inventory },
		function(buffer, pinfo, tree, t)
...
metaworld_server_commands[0x28] = { "OBJECTDATA", 2 }
metaworld_server_obsolete[0x28] = true

-- TOCLIENT_TIME_OF_DAY

do
	local f_time = ProtoField.uint16("metaworld.server.time_of_day", "Time", base.DEC)
	local f_time_speed = ProtoField.float("metaworld.server.time_speed", "Time Speed", base.DEC)

	metaworld_server_commands[0x29] = {
		"TIME_OF_DAY", 4,
		{ f_time, f_time_speed },
		function(buffer, pinfo, tree, t)
...
metaworld_server_commands[0x2a] = { "CSM_RESTRICTION_FLAGS", 2 }

-- TOCLIENT_PLAYER_SPEED

metaworld_server_commands[0x2b] = { "PLAYER_SPEED", 2 }

-- TOCLIENT_CHAT_MESSAGE

do
	local abbr = "metaworld.server.chat_message_"
	local vs_type = {
		[0] = "Raw",
		[1] = "Normal",
		[2] = "Announce",
	...
	local f_senderlen, f_sender = metaworld_field_helper("uint16", abbr.."sender",
		"Message sender")
	local f_messagelen, f_message = metaworld_field_helper("uint16", abbr:sub(1,-2),
		"Message")

	metaworld_server_commands[0x2f] = {
		"CHAT_MESSAGE", 8,
		...
		function(buffer, pinfo, tree, t)
			...
			local off = 4
			off = metaworld_decode_helper_utf16(buffer, t, "uint16", off, f_senderlen, f_sender)
			if off then
				off = metaworld_decode_helper_utf16(buffer, t, "uint16", off, f_messagelen, f_message)
			end
		end
	}
end
...
metaworld_server_commands[0x30] = { "CHAT_MESSAGE_OLD", 2 }
metaworld_server_obsolete[0x30] = true

-- TOCLIENT_ACTIVE_OBJECT_REMOVE_ADD

do
	local f_removed_count = ProtoField.uint16(
		"metaworld.server.active_object_remove_add_removed_count",
		"Count of removed objects", base.DEC)
	local f_removed = ProtoField.bytes(
		"metaworld.server.active_object_remove_add_removed",
		"Removed object")
	local f_removed_id = ProtoField.uint16(
		"metaworld.server.active_object_remove_add_removed_id",
		"ID", base.DEC)

	local f_added_count = ProtoField.uint16(
		"metaworld.server.active_object_remove_add_added_count",
		"Count of added objects", base.DEC)
	local f_added = ProtoField.bytes(
		"metaworld.server.active_object_remove_add_added",
		"Added object")
	local f_added_id = ProtoField.uint16(
		"metaworld.server.active_object_remove_add_added_id",
		"ID", base.DEC)
	local f_added_type = ProtoField.uint8(
		"metaworld.server.active_object_remove_add_added_type",
		"Type", base.DEC)
	local f_added_init_length = ProtoField.uint32(
		"metaworld.server.active_object_remove_add_added_init_length",
		"Initialization data length", base.DEC)
	local f_added_init_data = ProtoField.bytes(
		"metaworld.server.active_object_remove_add_added_init_data",
		"Initialization data")

	metaworld_server_commands[0x31] = {
		...
		function(buffer, pinfo, tree, t)
			...
			if not metaworld_check_length(buffer, added_count_pos + 2, t) then
				return
			end
			...
			for index = 0, added_count - 1 do
				if not metaworld_check_length(buffer, pos + 7, t) then
					return
				end

				local init_length = buffer(pos + 3, 4):uint()
				if not metaworld_check_length(buffer, pos + 7 + init_length, t) then
					return
				end
...
do
	local f_object_count = ProtoField.uint16(
		"metaworld.server.active_object_messages_object_count",
		"Count of objects", base.DEC)
	local f_object = ProtoField.bytes(
		"metaworld.server.active_object_messages_object",
		"Object")
	local f_object_id = ProtoField.uint16(
		"metaworld.server.active_object_messages_id",
		"ID", base.DEC)
	local f_message_length = ProtoField.uint16(
		"metaworld.server.active_object_messages_message_length",
		"Message length", base.DEC)
	local f_message = ProtoField.bytes(
		"metaworld.server.active_object_messages_message",
		"Message")

	metaworld_server_commands[0x32] = {
		...
		function(buffer, pinfo, tree, t)
			...
			while pos < buffer:len() do
				if not metaworld_check_length(buffer, pos + 4, t) then
					return
				end
				message_length = buffer(pos + 2, 2):uint()
				if not metaworld_check_length(buffer, pos + 4 + message_length, t) then
					return
				end
				count = count + 1
...
do
	local f_hp = ProtoField.uint16("metaworld.server.hp", "Health points", base.DEC)

	metaworld_server_commands[0x33] = {
		"HP", 4,
		{ f_hp },
		function(buffer, pinfo, tree, t)
...
do
	local abbr = "metaworld.server.move_player_"
	...
	metaworld_server_commands[0x34] = {
		"MOVE_PLAYER", 22,
...
do
	local f_reason_length = ProtoField.uint16("metaworld.server.access_denied_reason_length", "Reason length", base.DEC)
	local f_reason = ProtoField.string("metaworld.server.access_denied_reason", "Reason")

	metaworld_server_commands[0x35] = {
		"ACCESS_DENIED_LEGACY", 4,
		{ f_reason_length, f_reason },
		function(buffer, pinfo, tree, t)
			t:add(f_reason_length, buffer(2,2))
			local reason_length = buffer(2,2):uint()
			if metaworld_check_length(buffer, 4 + reason_length * 2, t) then
				t:add(f_reason, metaworld_convert_utf16(buffer(4, reason_length * 2), "Converted reason message"))
			end
		end
	}
end
...
metaworld_server_commands[0x38] = {"MEDIA", 2}

-- TOCLIENT_TOOLDEF (obsolete)

metaworld_server_commands[0x39] = {"TOOLDEF", 2}
metaworld_server_obsolete[0x39] = true

-- TOCLIENT_NODEDEF

metaworld_server_commands[0x3a] = {"NODEDEF", 2}

-- TOCLIENT_CRAFTITEMDEF (obsolete)

metaworld_server_commands[0x3b] = {"CRAFTITEMDEF", 2}
metaworld_server_obsolete[0x3b] = true

-- ...

metaworld_server_commands[0x3c] = {"ANNOUNCE_MEDIA", 2}
metaworld_server_commands[0x3d] = {"ITEMDEF", 2}
metaworld_server_commands[0x3f] = {"PLAY_SOUND", 2}
metaworld_server_commands[0x40] = {"STOP_SOUND", 2}
metaworld_server_commands[0x41] = {"PRIVILEGES", 2}
metaworld_server_commands[0x42] = {"INVENTORY_FORMSPEC", 2}
metaworld_server_commands[0x43] = {"DETACHED_INVENTORY", 2}
metaworld_server_commands[0x44] = {"SHOW_FORMSPEC", 2}
metaworld_server_commands[0x45] = {"MOVEMENT", 2}
metaworld_server_commands[0x46] = {"SPAWN_PARTICLE", 2}
metaworld_server_commands[0x47] = {"ADD_PARTICLE_SPAWNER", 2}

-- TOCLIENT_DELETE_PARTICLESPAWNER_LEGACY (obsolete)

metaworld_server_commands[0x48] = {"DELETE_PARTICLESPAWNER_LEGACY", 2}
metaworld_server_obsolete[0x48] = true

-- ...

metaworld_server_commands[0x49] = {"HUDADD", 2}
metaworld_server_commands[0x4a] = {"HUDRM", 2}
metaworld_server_commands[0x4b] = {"HUDCHANGE", 2}
metaworld_server_commands[0x4c] = {"HUD_SET_FLAGS", 2}
metaworld_server_commands[0x4d] = {"HUD_SET_PARAM", 2}
metaworld_server_commands[0x4e] = {"BREATH", 2}
metaworld_server_commands[0x4f] = {"SET_SKY", 2}
metaworld_server_commands[0x50] = {"OVERRIDE_DAY_NIGHT_RATIO", 2}
metaworld_server_commands[0x51] = {"LOCAL_PLAYER_ANIMATIONS", 2}
metaworld_server_commands[0x52] = {"EYE_OFFSET", 2}
metaworld_server_commands[0x53] = {"DELETE_PARTICLESPAWNER", 2}
metaworld_server_commands[0x54] = {"CLOUD_PARAMS", 2}
metaworld_server_commands[0x55] = {"FADE_SOUND", 2}

-- TOCLIENT_UPDATE_PLAYER_LIST

do
	local abbr = "metaworld.server.update_player_list_"
	local vs_type = {
		[0] = "Init",
		[1] = "Add",
		[2] = "Remove",
	...
	metaworld_server_commands[0x56] = {
		"UPDATE_PLAYER_LIST",
		5,
		...
		function(buffer, pinfo, tree, t)
			...
			local off = 5
			for i = 1, count do
				if not metaworld_check_length(buffer, off + 2, t) then
					return
				end
				off = metaworld_decode_helper_ascii(buffer, t, "uint16", off, nil, f_name)
				if not off then
					return
				end
			end
...
metaworld_server_commands[0x57] = {"MODCHANNEL_MSG", 2}
metaworld_server_commands[0x58] = {"MODCHANNEL_SIGNAL", 2}
metaworld_server_commands[0x59] = {"NODEMETA_CHANGED", 2}
metaworld_server_commands[0x5a] = {"SET_SUN", 2}
metaworld_server_commands[0x5b] = {"SET_MOON", 2}
metaworld_server_commands[0x5c] = {"SET_STARS", 2}
metaworld_server_commands[0x60] = {"SRP_BYTES_S_B", 2}
metaworld_server_commands[0x61] = {"FORMSPEC_PREPEND", 2}
...
-- metaworld.control dissector

do
	local p_control = Proto("metaworld.control", "metaworld Control")
	...
	local f_control_type = ProtoField.uint8("metaworld.control.type", "Control Type", base.DEC, vs_control_type)
	local f_control_ack = ProtoField.uint16("metaworld.control.ack", "ACK sequence number", base.DEC)
	local f_control_peerid = ProtoField.uint8("metaworld.control.peerid", "New peer ID", base.DEC)
...
-- metaworld.client dissector
-- metaworld.server dissector

-- Defines the metaworld.client or metaworld.server Proto. These two protocols
...
function metaworld_define_client_or_server_proto(is_client)
	-- Differences between metaworld.client and metaworld.server
	local proto_name, this_peer, other_peer, empty_message_info
	local commands, obsolete
	if is_client then
		proto_name = "metaworld.client"
		this_peer = "Client"
		other_peer = "Server"
		empty_message_info = "Empty message / Connect"
		commands = metaworld_client_commands  -- defined in Part 2
		obsolete = metaworld_client_obsolete  -- defined in Part 2
	else
		proto_name = "metaworld.server"
		this_peer = "Server"
		other_peer = "Client"
		empty_message_info = "Empty message"
		commands = metaworld_server_commands  -- defined in Part 3
		obsolete = metaworld_server_obsolete  -- defined in Part 3
	end

	-- Create the protocol object.
	local proto = Proto(proto_name, "metaworld " .. this_peer .. " to " .. other_peer)
	...
	-- metaworld.client or metaworld.server dissector function
	function proto.dissector(buffer, pinfo, tree)
		...
		elseif metaworld_check_length(buffer, 2, t) then
			...
			if command_info == nil then
				...
				if metaworld_check_length(buffer, command_min_length, t) then
...
metaworld_define_client_or_server_proto(true)  -- metaworld.client
metaworld_define_client_or_server_proto(false) -- metaworld.server

-- metaworld.split dissector

do
	local p_split = Proto("metaworld.split", "metaworld Split Message")

	local f_split_seq = ProtoField.uint16("metaworld.split.seq", "Sequence number", base.DEC)
	local f_split_chunkcount = ProtoField.uint16("metaworld.split.chunkcount", "Chunk count", base.DEC)
	local f_split_chunknum = ProtoField.uint16("metaworld.split.chunknum", "Chunk number", base.DEC)
	local f_split_data = ProtoField.bytes("metaworld.split.data", "Split message data")
...
do
	local p_metaworld = Proto("metaworld", "metaworld")

	local metaworld_id = 0x4f457403
	local vs_id = {
		[metaworld_id] = "Valid"
	}
	...
	local f_id = ProtoField.uint32("metaworld.id", "ID", base.HEX, vs_id)
	local f_peer = ProtoField.uint16("metaworld.peer", "Peer", base.DEC, vs_peer)
	local f_channel = ProtoField.uint8("metaworld.channel", "Channel", base.DEC)
	local f_type = ProtoField.uint8("metaworld.type", "Type", base.DEC, vs_type)
	local f_seq = ProtoField.uint16("metaworld.seq", "Sequence number", base.DEC)
	local f_subtype = ProtoField.uint8("metaworld.subtype", "Subtype", base.DEC, vs_type)

	p_metaworld.fields = { f_id, f_peer, f_channel, f_type, f_seq, f_subtype }

	local data_dissector = Dissector.get("data")
	local control_dissector = Dissector.get("metaworld.control")
	local client_dissector = Dissector.get("metaworld.client")
	local server_dissector = Dissector.get("metaworld.server")
	local split_dissector = Dissector.get("metaworld.split")

	function p_metaworld.dissector(buffer, pinfo, tree)

		-- Add metaworld tree item and verify the ID
		local t = tree:add(p_metaworld, buffer(0,8))
		t:add(f_id, buffer(0,4))
		if buffer(0,4):uint() ~= metaworld_id then
			t:add_expert_info(PI_UNDECODED, PI_WARN, "Invalid ID, this is not a metaworld packet")
			return
		end

		-- ID is valid, so replace packet's shown protocol
		pinfo.cols.protocol = "metaworld"
		pinfo.cols.info = "metaworld"

		-- Set the other header fields
		t:add(f_peer, buffer(4,2))
		t:add(f_channel, buffer(6,1))
		t:add(f_type, buffer(7,1))
		t:set_text("metaworld, Peer: " .. buffer(4,2):uint() .. ", Channel: " .. buffer(6,1):uint())
	...
	DissectorTable.get("udp.port"):add(30000, p_metaworld)
	DissectorTable.get("udp.port"):add(30001, p_metaworld)
end
...
function metaworld_check_length(tvb, min_len, t)
...
-- t_textlen, t_text should be the ProtoFields created by metaworld_field_helper
--   alternatively t_text can be a ProtoField.string and t_textlen can be nil
-- lentype must be the type of the length field (as passed to metaworld_field_helper)
-- returns nil if length check failed
function metaworld_decode_helper_ascii(tvb, t, lentype, offset, f_textlen, f_text)
	...
	if metaworld_check_length(tvb, offset + n + textlen, t) then
...
-- (see metaworld_decode_helper_ascii)
function metaworld_decode_helper_utf16(tvb, t, lentype, offset, f_textlen, f_text)
	local n = ({uint16 = 2, uint32 = 4})[lentype]
	assert(n)
