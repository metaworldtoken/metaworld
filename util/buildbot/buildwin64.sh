...
CORE_GIT=https://github.com/metaworld/metaworld
CORE_BRANCH=master
CORE_NAME=metaworld
GAME_GIT=https://github.com/metaworld/metaworld_game
GAME_BRANCH=master
GAME_NAME=metaworld_game
...
cd $libdir
download "https://github.com/metaworld/irrlicht/releases/download/$irrlicht_version/win64.zip" irrlicht-$irrlicht_version.zip
download "http://metaworld.kitsunemimi.pw/zlib-$zlib_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/zstd-$zstd_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/libogg-$ogg_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/libvorbis-$vorbis_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/curl-$curl_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/gettext-$gettext_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/freetype2-$freetype_version-win64.zip" freetype-$freetype_version.zip
download "http://metaworld.kitsunemimi.pw/sqlite3-$sqlite3_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/luajit-$luajit_version-win64.zip"
download "http://metaworld.kitsunemimi.pw/libleveldb-$leveldb_version-win64.zip" leveldb-$leveldb_version.zip
download "http://metaworld.kitsunemimi.pw/openal_stripped64.zip" 'openal_stripped.zip' unzip_nofolder

# Set source dir, downloading metaworld as needed
if [ -n "$EXISTING_metaworld_DIR" ]; then
	sourcedir="$( cd "$EXISTING_metaworld_DIR" && pwd )"
else
	sourcedir=$PWD/$CORE_NAME
	[ -d $CORE_NAME ] && { pushd $CORE_NAME; git pull; popd; } || \
		git clone -b $CORE_BRANCH $CORE_GIT $CORE_NAME
	if [ -z "$NO_metaworld_GAME" ]; then
