...
perform_release() {
	...
	sed -i '/\<release/s/\(version\)="[^"]*"/\1="'"$RELEASE_VERSION"'"/' misc/net.metaworld.metaworld.appdata.xml
	sed -i 's/\(<release date\)="[^"]*"/\1="'"$RELEASE_DATE"'"/' misc/net.metaworld.metaworld.appdata.xml

	git add -f CMakeLists.txt android/build.gradle misc/net.metaworld.metaworld.appdata.xml
...
##################################
# Switch to top metaworld directory
##################################
...
echo "Current metaworld version: $RELEASE_VERSION"
