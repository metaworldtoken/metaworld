...
bool RenderingEngine::setupTopLevelWindow(const std::string &name)
{
	...
	// the environment metaworld is running in.
...
void RenderingEngine::setupTopLevelXorgWindow(const std::string &name)
{
	...
	// the _NET_WM_PID (see further down) but running metaworld in an env
	// where the window manager is on another machine from metaworld (therefore
	...
	// Set the WM_CLIENT_LEADER window property here. metaworld has only one
