...
{
	const char *saved_env_mt_subgame_path = getenv("metaworld_SUBGAME_PATH");
	const char *saved_env_mt_mod_path = getenv("metaworld_MOD_PATH");
#ifdef WIN32
	{
		std::string subgame_path("metaworld_SUBGAME_PATH=");
		subgame_path.append(TEST_SUBGAME_PATH);
		_putenv(subgame_path.c_str());

		std::string mod_path("metaworld_MOD_PATH=");
		mod_path.append(TEST_MOD_PATH);
		_putenv(mod_path.c_str());
	}
#else
	setenv("metaworld_SUBGAME_PATH", TEST_SUBGAME_PATH, 1);
	setenv("metaworld_MOD_PATH", TEST_MOD_PATH, 1);
#endif
...
#else
	if (saved_env_mt_subgame_path)
		setenv("metaworld_SUBGAME_PATH", saved_env_mt_subgame_path, 1);
	else
		unsetenv("metaworld_SUBGAME_PATH");
	if (saved_env_mt_mod_path)
		setenv("metaworld_MOD_PATH", saved_env_mt_mod_path, 1);
	else
		unsetenv("metaworld_MOD_PATH");
#endif
}
...
void TestServerModManager::testGetMods()
{
	...
	// and test_mod (for testing metaworld_MOD_PATH).
