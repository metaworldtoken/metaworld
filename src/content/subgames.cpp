...
bool getGamemetaworldConfig(const std::string &game_path, Settings &conf)
{
	std::string conf_path = game_path + DIR_DELIM + "metaworld.conf";
...
{
	char *subgame_path = getenv("metaworld_SUBGAME_PATH");
...
#define LEGACY_GAMEID "metaworld"
...
std::string getWorldGameId(const std::string &world_path, bool can_be_legacy)
{
	...
	if (!succeeded) {
		if (can_be_legacy) {
			// If map_meta.txt exists, it is probably an old metaworld world
...
{
	char *world_path = getenv("metaworld_WORLD_PATH");
		...
		const SubgameSpec &gamespec, bool create_world)
{
	...
	getGamemetaworldConfig(gamespec.path, *game_settings);
...
{
	const char *c_mod_path = getenv("metaworld_MOD_PATH");
